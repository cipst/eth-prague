...
task("lz:oft:receive", "Receive tokens")
	...
	.setAction(async (taskArgs: Args, { ethers, deployments }) => {
		...
		const feeQuote = await oft.quoteOFT(sendParam);
		...
		const r = await oft.lzReceive(sendParam, { nativeFee: nativeFee, lzTokenFee: 0 }, signer.address, {
			value: nativeFee,
		});
